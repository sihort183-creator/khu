...
- `khu-backup` — 주기 7일, 유예 1일 → 주소를 `KHU_BACKUP_HEARTBEAT_URL` 로 등록

수집이나 백업이 멈추면 메일이 온다. 백업은 무료 요금제의 유일한 복구 수단이므로 이 감시를 붙이는 것을 권한다.

### 백업 주기 (2026-09-09 변경: 6시간 → 주 1회)

백업 워크플로는 **한국 시간 일요일 04:52**(cron `52 19 * * 6`, UTC 토요일 19:52)에 주 1회 돈다.
매시 17분에 시작해 20~25분 도는 수집 회차, 15분마다 도는 수집 감시와 겹치지 않는 시각이다.

바꾼 이유는 요금이다. 백업은 `pg_dump` 로 데이터베이스 전체를 읽으므로 한 번에 원본 약 125MB
(압축·암호화 뒤 약 28MB 를 R2 에 올린다)가 데이터베이스에서 나간다. 하루 4번이면 하루 0.5GB,
한 달 약 15GB 로 Supabase 무료 egress 한도(월 5GB)의 세 배다. 2026-09-09 Supabase 가 한도 초과를
통지했고 10월 9일부터 제한이 걸릴 수 있다. 주 1회면 하루 약 18MB 로 내려간다.

**부작용: 복구 시 최대 7일치 운영 기록을 잃는다.** 마지막 백업 이후에 생긴 실행 이력(`runs`,
`source_runs`), 출처 상태(`source_health`, 재시도 위치·백필 진행), 중복 판정 기록, 감사 기록,
사용자 설정은 복원되지 않는다. 복구 뒤에는 출처 상태가 최대 일주일 전으로 돌아가므로 첫 회차가
그만큼 더 오래 걸린다.

**공지 자체는 백업이 아니어도 남는다.** 매시 수집이 공개용 JSON 전체 사본을 R2 공개 버킷에
올리고, 원문 증거는 증거 버킷에 남는다. 즉 공지·본문·연락처는 R2 파일에서 되살릴 수 있고,
이 백업이 지키는 것은 위의 운영 기록 쪽이다.

**보존 정책은 그대로 두었다.** 최근 7일 안의 백업은 모두, 그 뒤 30일까지 하루 1개를 남기고
30일이 지나면 지운다(`backend/app/run/backup.py` 의 `RECENT_DAYS`, `DAILY_DAYS`). 주 1회로
바뀌면 실제로 남는 것은 **주 1개씩 약 4~5개**다. 상수를 늘리면 탈퇴·삭제 요청이 백업에 남는
기간도 함께 늘어나므로 늘리지 않았다. `--prune` 은 워크플로가 돌 때만 동작하고, 정책을 바꾸지
않았으므로 **이 변경 때문에 지금 R2 에 있는 백업이 추가로 지워지는 일은 없다.** 다음 실행에서
지워지는 것은 예전 규칙에서도 지워질 것들, 즉 8~30일 된 같은 날짜의 6시간 단위 백업 중
하루 1개를 남긴 나머지와, 30일이 지난 백업뿐이다.

2026-09-09 기준 백업 버킷에는 11개(2026-09-06 ~ 09-09)가 있다. 이 목록에 보존 정책을
그대로 적용해 보면 다음과 같다.

| 실행 시각(UTC) | 지워지는 백업 | 남는 백업 |
|---|---|---|
| 2026-09-12 19:52 (첫 주 1회 실행) | 0개 — 모두 7일 안이다 | 11 + 새 백업 1 |
| 2026-09-19 19:52 | 7개 — 09-06~09-08 의 같은 날짜 중복분 | 하루 1개씩 + 최근 것 |
| 2026-09-26 19:52 | 추가 0개 | 그대로 |
